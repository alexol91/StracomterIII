...
func test_remake_and_chutaos_themes_are_distinct_resources() -> void:
	assert_ne(UiStyle.remake_theme(), UiStyle.chutaos_theme())


func test_theme_builders_are_cached_singletons() -> void:
	assert_eq(UiStyle.remake_theme(), UiStyle.remake_theme())
...
func test_title_label_variation_uses_a_much_larger_font_size_than_body() -> void:
	var theme := UiStyle.remake_theme()
...
func test_apply_snapshot_picks_remake_theme_when_chutaos_is_off() -> void:
	PresentationStyle.chutaos_mode = false
	_node = Control.new()
	UiStyle.apply_snapshot(_node)
	assert_eq(_node.theme, UiStyle.remake_theme())
