#!/usr/bin/env bash
# Sonda de rendimiento. Ver tools/perf_probe/probe.gd.
#
#   tools/perf_probe/probe.sh <ruta-a-godot>
#
...
SCRIPT="${ROOT}/game/_perf_probe_tmp.gd"
SCENE="${ROOT}/game/_perf_probe_tmp.tscn"

cleanup() { rm -f "${SCRIPT}" "${SCRIPT}.uid" "${SCENE}"; }
trap cleanup EXIT

cp "${ROOT}/tools/perf_probe/probe.gd" "${SCRIPT}"
cat > "${SCENE}" <<'TSCN'
[gd_scene load_steps=2 format=3]

[ext_resource type="Script" path="res://_perf_probe_tmp.gd" id="1"]

[node name="PerfProbe" type="Node"]
